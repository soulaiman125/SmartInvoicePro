...
function drawFooters(doc, t, org, footerText) {
  ...
  const contact = [s.companyPhone, s.companyEmail, s.companyWebsite].filter(Boolean).join('   ·   ');
  ...
  for (let i = 0; i < range.count; i += 1) {
    ...
    doc.text(footerText || `${org?.name || 'SmartInvoice Pro'} · Thank you for your business`, LEFT, FOOTER_TOP + 8, { width: CONTENT_W * 0.62 });
    doc.text(`Page ${i + 1} of ${range.count}`, RIGHT - 120, FOOTER_TOP + 8, { width: 120, align: 'right' });
    // Company contact line (phone · email · website) when configured.
    if (contact) {
      doc.fillColor(t.muted).fontSize(7.5).text(contact, LEFT, FOOTER_TOP + 21, { width: CONTENT_W, align: 'center' });
    }
    doc
      .fillColor('#9CA3AF')
      .fontSize(7)
      .text('Generated by SmartInvoice Pro', LEFT, FOOTER_TOP + (contact ? 32 : 22), { width: CONTENT_W, align: 'center' });
  }
}
